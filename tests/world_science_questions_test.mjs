...
assert.equal(new Set(all.map(question => normalizeArabic(question.q))).size, 540);
assert.equal(new Set(all.map(question => question.factKey)).size, 540);
...
console.log('✓ وحدة العالم والعلوم: 540 سؤالًا متدرجًا، متباين السجلات، ومقاومًا للعبث');
