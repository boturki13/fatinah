...
- **Highest-risk areas**: promo redemption (`/api/promo/redeem`, line ~963), checkout creation (`/api/stripe/create-checkout`, line ~795), account delete/profile (`/api/account/delete`, `/api/account/profile`)
...
**Required guarantee**: Status endpoints that expose per-user data SHOULD require at minimum a valid session token. Exception details MUST NOT be sent verbatim to clients.
